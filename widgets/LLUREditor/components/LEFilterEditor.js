...
define(["dojo/_base/declare", "dojo/_base/lang", "dojo/_base/array", "dojo/dom-construct", "dijit/_TemplatedMixin", "dijit/_WidgetBase"], function (declare, lang, array, domConstruct, _TemplatedMixin, _WidgetBase) {
  return declare([_WidgetBase, _TemplatedMixin], {
    ...
    declaredClass: 'jimu.dijit.LEFilterEditor',
    templateString: "<div style='width:100%'>" + "<div data-dojo-attach-point='filterEditorDiv'></div></div>",
    ...
    _createFilterTool: function _createFilterTool() {
      // label for select
    ...
    removeOptions: function removeOptions(selectbox) {
      var i;
    ...
    _loadTemplates: function _loadTemplates() {
      var selectedValue = this.selectDropDown.value === this.nls.filterEditor.all || this.selectDropDown.value === "" ? null : this.selectDropDown.value;
    ...
    setTemplatePicker: function setTemplatePicker(templatePicker, layers) {
      this._layers = layers;
      this._templatePicker = templatePicker;
      this._overrideTemplatePicker();
      this._loadTemplates();
    },
    _createTemplateFilter: function _createTemplateFilter() {
      // textBox filter
    ...
    _overrideTemplatePicker: function _overrideTemplatePicker() {
      ...
      this._templatePicker._getItemsFromLayer = lang.hitch(this, function () {
          ...
          items = array.filter(items, function (item) {
            ...
            array.forEach(filterVals, function (filterValText) {
              if (filterValText.length > 1) {
                ...
                var regex = new RegExp("^" + filterValText + "$", 'i');
        ...
        if (items.length === 0) {
          this._templatePicker.grid.noDataMessage = this.nls.filterEditor.noAvailableTempaltes;
        }
        return items;
      });
    },
     ...
     **/
    _onLayerFilterChanged: function _onLayerFilterChanged() {
      ...
      if (val !== "") {
        if (val === this.nls.filterEditor.all) {
          ...
          this._templatePicker.attr("featureLayers", visLayers);
    ...
    _onTemplateFilterChanged: function _onTemplateFilterChanged() {
      var val = this.selectDropDown.options[this.selectDropDown.selectedIndex].text;
      var filterText = this.filterTextBox.value;
