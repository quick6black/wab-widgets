...
define(["dojo/_base/declare", "dojo/Evented", "dojo/_base/lang", "dojo/_base/array", 'dojo/dom-construct', 'dojo/on', "dojo/text!./Coordinates.html", 'dijit/_TemplatedMixin', 'jimu/BaseWidgetSetting', "jimu/dijit/Popup", "dijit/form/Select", 'dijit/form/ValidationTextBox', "./layersAndFieldsApplyOn", 'dojo/dom-class'], function (declare, Evented, lang, array, domConstruct, on, template, _TemplatedMixin, BaseWidgetSetting, Popup, Select, TextBox, layersAndFieldsApplyOn, domClass) {
  return declare([BaseWidgetSetting, Evented, _TemplatedMixin], {
    ...
    showDialog: function showDialog() {
      var width, isEnabled, coordinatesSystem, field, coordinateSystemSelector, fieldsPopup, maxHeight, autoHeight;
      isEnabled = false;
      width = 675;
      maxHeight = 500;
      autoHeight = false;
    ...
    */
    _createCoordinatesOptions: function _createCoordinatesOptions() {
      var options = [];
    ...
    * @memberOf setting/Coordinates
    */
    _getFieldsOptionsObj: function _getFieldsOptionsObj(coordinatesSystem) {
      var fieldOptions = [],
          fieldArray = [];
      if (coordinatesSystem === "LatLong") {
        fieldArray = [{ "name": "y", "alias": "Latitude" }, { "name": "x", "alias": "Longitude" }, { "name": "xy", "alias": "Latitude Longitude" }];
      } else {
        fieldArray = [{ "name": "x", "alias": "X" }, { "name": "y", "alias": "Y" }, { "name": "xy", "alias": "X Y" }];
      }
      array.forEach(fieldArray, lang.hitch(this, function (field) {
        //In case of group add filed option only when it is not used
        if (this.isGroup) {
          if (this.coordinatesSavedDataTypes.MapSpatialReference.indexOf(field.alias) !== -1 || this.coordinatesSavedDataTypes.LatLong.indexOf(field.alias) !== -1) {
            fieldOptions.push({
          ...
          if (field.name !== "xy" || field.name === "xy" && this._fieldType === "esriFieldTypeString") {
            fieldOptions.push({
              "label": field.alias || field.name,
              "value": field.name
            });
    ...
    _createLayerFieldsFilter: function _createLayerFieldsFilter() {
      ...
      if (this.fieldSelector && this.fieldSelector.value === "xy") {
        ...
        array.forEach(this._totalLayers, lang.hitch(this, function (layer) {
          if (!layer.isTable) {
            array.forEach(layer.layerObject.fields, lang.hitch(this, function (field) {
              if (field.type === "esriFieldTypeString" || field.type === "esriFieldTypeSmallInteger" || field.type === "esriFieldTypeInteger" || field.type === "esriFieldTypeSingle" || field.type === "esriFieldTypeDouble" && field.editable) {
                if (!layerDetails[layer.id]) {
                  layerDetails[layer.id] = {};
                }
                layerDetails[layer.id][field.name] = field;
