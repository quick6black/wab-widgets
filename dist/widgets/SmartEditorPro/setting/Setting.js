...
define(['dojo/_base/declare', 'dijit/_WidgetsInTemplateMixin', 'jimu/BaseWidgetSetting', 'jimu/dijit/SimpleTable', 'jimu/LayerInfos/LayerInfos', 'dojo/_base/lang', 'dojo/on', 'dojo/query', 'dijit/registry', 'dojo/_base/array', "./EditFields", "../PresetAllFields", "./EditDescription", "./SmartActionGroup", "../utils", 'dijit/Editor', 'dojo/dom-style', 'dojo/dom-attr', 'dojo/dom-class', 'dojo/sniff', 'jimu/utils', 'dojo/_base/html', "jimu/dijit/_GeocodeServiceChooserContent", "jimu/dijit/Popup", "esri/request", "esri/lang", 'jimu/dijit/Message', "jimu/dijit/LoadingShelter", 'jimu/dijit/LoadingIndicator', 'jimu/dijit/TabContainer3', 'dojo/dom-construct', 'dojo/promise/all', 'dojo/Deferred', 'jimu/portalUtils', './Intersection', './Coordinates', './Address', './Preset', 'jimu/dijit/LayerChooserFromMap', 'dijit/_editor/plugins/LinkDialog', 'dijit/_editor/plugins/ViewSource', 'dijit/_editor/plugins/FontChoice', 'dojox/editor/plugins/Preview', 'dijit/_editor/plugins/TextColor', 'dojox/editor/plugins/ToolbarLineBreak', 'dojox/editor/plugins/FindReplace', 'dojox/editor/plugins/PasteFromWord', 'dojox/editor/plugins/InsertAnchor', 'dojox/editor/plugins/Blockquote', 'dojox/editor/plugins/UploadImage', 'jimu/dijit/EditorChooseImage', 'jimu/dijit/EditorTextColor', 'jimu/dijit/EditorBackgroundColor', 'dijit/form/CheckBox', 'jimu/dijit/RadioBtn'], function (declare, _WidgetsInTemplateMixin, BaseWidgetSetting, Table, LayerInfos, lang, on, query, registry, array, EditFields, PresetAllFields, EditDescription, SmartActionGroup, editUtils, Editor, domStyle, domAttr, domClass, has, utils, html, _GeocodeServiceChooserContent, Popup, esriRequest, esriLang, Message, LoadingShelter, LoadingIndicator, TabContainer3, domConstruct, all, Deferred, portalUtils, Intersection, Coordinates, Address, Preset, LayerChooserFromMap) {

  return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
    baseClass: 'jimu-widget-smartEditor-pro-setting',
    ...
    _processConfig: function _processConfig() {
      var configInfos;
      if (!this.config.editor.hasOwnProperty("presetInfos") && this.config.editor.layerInfos) {
    ...
    **/
    ...
    _initLayersTable: function _initLayersTable() {
      var fields = [{
        name: 'edit',
      ...
      nl.forEach(function (node) {
        var scrubText = node.innerText === undefined || node.innerText === "" ? "" : node.innerText.replace(/(\r\n|\n|\r)/gm, "");
        switch (scrubText) {
      ...
      this.own(on(this._layersTable, 'actions-edit', lang.hitch(this, this._onEditFieldInfoClick)));
    },
    _createSpecialType: function _createSpecialType(td) {
    ...
    _setValue4SpecialType: function _setValue4SpecialType() {
      //var select = query('select', td)[0];
      //select.value = value;
    },

    _getValueOfSpecialType: function _getValueOfSpecialType() {
      //var select = query('select', td)[0];
      //return select.value;
    },
    ...
    setConfig: function setConfig() {
      ...
      this._fetchDefaultGeocoder().then(lang.hitch(this, function (defaultGeocoder) {
        ...
        this._getTableInfos().then(lang.hitch(this, function () {
          this._configInfos = editUtils.getConfigInfos(this._jimuLayerInfos, this.config.editor.layerInfos, false, false);
    ...
    _fetchOrgsHelperServiecs: function _fetchOrgsHelperServiecs() {
      ...
      if (this.appConfig.portalUrl && lang.trim(this.appConfig.portalUrl) !== "") {
        ...
        portalUtils.getPortalSelfInfo(this.appConfig.portalUrl).then(lang.hitch(this, function (portalInfo) {
          ...
          if (helperServices && helperServices.geocode && helperServices.geocode.length > 0 && helperServices.geocode[0].url) {
    ...
    _fetchDefaultGeocoder: function _fetchDefaultGeocoder() {
      var def = new Deferred();
    ...
    _getTableInfos: function _getTableInfos() {
      var defs = [];
    ...
    */
    ...
    _setLayersTable: function _setLayersTable(configuredLayerInfos, isRelatedInfo) {
      var nl = null,
          isAtLeastOneTable = false;
      if (isRelatedInfo) {
      ...
      array.forEach(configuredLayerInfos, function (configInfo) {
        ...
        var addRowResult = this._layersTable.addRow({
          ...
          allowUpdateOnlyHidden: configInfo.allowUpdateOnly === null ? false : configInfo.allowUpdateOnly,
          allowDelete: configInfo.allowDelete,
          allowDeleteHidden: configInfo.allowDelete === null ? false : configInfo.allowDelete,
          ...
          disableGeometryUpdateHidden: configInfo.disableGeometryUpdate === null ? false : configInfo.disableGeometryUpdate
        });
        ...
        if (configInfo.featureLayer.layerAllowsUpdate === false && configInfo.featureLayer.layerAllowsCreate === false) {
        ...
        if (relationshipInfo.length > 0) {
          ...
          if (!relationshipInfo[0].layerInfo) {
            ...
            relationshipInfo = editUtils.getConfigInfos(this._jimuLayerInfos, relationshipInfo, false, true, true);
    ...
    _onDescriptionClick: function _onDescriptionClick(tr) {
    ...
    _onEditFieldInfoClick: function _onEditFieldInfoClick(tr) {
      var rowData = this._layersTable.getRowData(tr);
      if (rowData && rowData.edit) {
    ...
    _initEditor: function _initEditor() {
      ...
      if (!this._editorObj) {
        ...
        this._editorObj = new Editor({
          plugins: ['bold', 'italic', 'underline', utils.getEditorTextColor("smartEditor"), utils.getEditorBackgroundColor("smartEditor"), '|', 'justifyLeft', 'justifyCenter', 'justifyRight', 'justifyFull', '|', 'insertOrderedList', 'insertUnorderedList', 'indent', 'outdent'],
          extraPlugins: ['|', 'createLink', 'unlink', 'pastefromword', '|', 'undo', 'redo', '|', 'toolbarlinebreak', //'chooseImage', 'uploadImage',
          {
    ...
    **/
    _initEditorPluginsCSS: function _initEditorPluginsCSS() {
    ...
    _resetSettingsConfig: function _resetSettingsConfig() {
      ...
      this.config.editor.useFilterEditor = this.useFilterEditor.checked === undefined ? false : this.useFilterEditor.checked;

      this.config.editor.displayShapeSelector = this.displayShapeSelector.checked === undefined ? false : this.displayShapeSelector.checked;

      this.config.editor.createNewFeaturesFromExisting = this.createNewFeaturesFromExisting.checked === undefined ? false : this.createNewFeaturesFromExisting.checked;

      this.config.editor.overrideDefaultsByCopiedFeature = this.overrideDefaultsByCopiedFeature.checked === undefined ? false : this.overrideDefaultsByCopiedFeature.checked;

      this.config.editor.displayPresetTop = this.displayPresetTop.checked === undefined ? false : this.displayPresetTop.checked;
      this.config.editor.listenToGF = this.listenToGF.checked === undefined ? false : this.listenToGF.checked;

      this.config.editor.keepTemplateSelected = this.keepTemplateSelected.checked === undefined ? false : this.keepTemplateSelected.checked;

      this.config.editor.editGeometryDefault = this.editGeometryDefault.checked === undefined ? false : this.editGeometryDefault.checked;

      this.config.editor.autoSaveEdits = this.autoSaveEdits.checked === undefined ? false : this.autoSaveEdits.checked;

      this.config.editor.enableAttributeUpdates = this.enableAttributeUpdates.checked === undefined ? false : this.enableAttributeUpdates.checked;

      this.config.editor.enableAutomaticAttributeUpdates = this.enableAutomaticAttributeUpdates.checked === undefined ? false : this.enableAutomaticAttributeUpdates.checked;

      this.config.editor.enableLockingMapNavigation = this.enableLockingMapNavigation.checked === undefined ? false : this.enableLockingMapNavigation.checked;

      this.config.editor.enableMovingSelectedFeatureToGPS = this.enableMovingSelectedFeatureToGPS.checked === undefined ? false : this.enableMovingSelectedFeatureToGPS.checked;

      this.config.editor.enableMovingSelectedFeatureToXY = this.enableMovingSelectedFeatureToXY.checked === undefined ? false : this.enableMovingSelectedFeatureToXY.checked;

      this.config.editor.showActionButtonsAbove = this.positionOfSaveButtonAbove.checked ? true : false;
    },

    _getConfigForCurrentDisplayedLayers: function _getConfigForCurrentDisplayedLayers() {
      ...
      array.forEach(this._currentConfigInfoInTable, function (configInfo, index) {
        ...
        configInfo.allowUpdateOnly = layersTableData[index].allowUpdateOnly === null ? layersTableData[index].allowUpdateOnlyHidden : layersTableData[index].allowUpdateOnly;
        configInfo.allowDelete = layersTableData[index].allowDelete === null ? layersTableData[index].allowDeleteHidden : layersTableData[index].allowDelete;
        configInfo.disableGeometryUpdate = layersTableData[index].disableGeometryUpdate === null ? layersTableData[index].disableGeometryUpdateHidden : layersTableData[index].disableGeometryUpdate;
      }, this);
    },

    _getCurrentConfigInfo: function _getCurrentConfigInfo(configInfos) {
      ...
      array.forEach(configInfos, function (configInfo) {
        ...
        if (configInfo.relationshipInfos && configInfo.relationshipInfos.length > 0) {
          configInfo.relationshipInfos = this._getCurrentConfigInfo(configInfo.relationshipInfos);
        }
      }, this);
    ...
    _resetFieldInfos: function _resetFieldInfos(fieldInfos) {
     ...
     **/
    _getRelatedTableInfo: function _getRelatedTableInfo(layer) {
      var tableInfos = [],
          baseURL,
          relationships,
          relationShipsWithoutDestination;
      if (layer) {
        ...
        relationShipsWithoutDestination = array.filter(relationships, function (relationInfo) {
        ...
        array.forEach(relationShipsWithoutDestination, lang.hitch(this, function (table) {
          ...
          array.forEach(this.map.webMapResponse.itemInfo.itemData.tables, lang.hitch(this, function (tableData) {
            var jimuTableInfo, fieldValidations, fieldValues;
            //once the url matches get its layer/table info using _jimuLayerInfos
            if (tableData.url.replace(/.*?:\/\//g, "") === (baseURL + table.relatedTableId).replace(/.*?:\/\//g, "")) {
          ...
          if (!foundInTables) {
            array.forEach(this.map.webMapResponse.itemInfo.itemData.operationalLayers, lang.hitch(this, function (layerData) {
              var jimuLayerInfo, fieldValidations, fieldValues;
              //once the url matches get its layer/table info using _jimuLayerInfos
              if (layerData.url.replace(/.*?:\/\//g, "") === (baseURL + table.relatedTableId).replace(/.*?:\/\//g, "")) {
                ...
                if (jimuLayerInfo) {
                  ...
                  fieldValidations = this._getSmartActionsForRelation(jimuLayerInfo.featureLayer.id);
                  if (!fieldValidations) {
                    fieldValidations = {};
                  }
     ...
     * @memberOf setting/Settings
     **/
    _addTableFieldActionIcon: function _addTableFieldActionIcon(tableRow, relationshipInfo) {
      ...
      this.own(on(tableRow.tableFieldDiv, 'click', lang.hitch(this, function () {
    ...
    _updateValidationsAccordingToGroups: function _updateValidationsAccordingToGroups(relationshipInfo) {
    ...
    _addBreadCrumb: function _addBreadCrumb(newTitle, isFirstLink) {
    ...
    _onBreadCrumbSectionClick: function _onBreadCrumbSectionClick(contentWrapper, evt) {
    ...
    _openServiceChooser: function _openServiceChooser(isGeocoderBtnClicked) {
      var contentDiv,
          prevConfiguredURL = "";
      ...
      this.serviceChooserContent.own(on(this.serviceChooserContent, 'ok', lang.hitch(this, function (evt) {
    ...
    _showPresetDialog: function _showPresetDialog() {
      ...
      this.own(on(presetAllFields, "PrestInfoUpdated", lang.hitch(this, function (configuredPresetInfos) {
    ...
    _onSelectLocatorUrlOk: function _onSelectLocatorUrlOk(evt, isGeocoderBtnClicked) {
    ...
    _initsmartActionsTable: function _initsmartActionsTable() {
      var fields = [{
        name: 'name',
      ...
      this.own(on(this._smartActionsTable, 'actions-edit', lang.hitch(this, this._configureGroupSmartAction)));
    ...
    _getSmartActionGroupConfig: function _getSmartActionGroupConfig() {
      var rows,
          smartActionGroups = {};
    ...
    _populateSmartActionGroups: function _populateSmartActionGroups() {
    ...
    _onDeleteSmartActionsClick: function _onDeleteSmartActionsClick(tr) {
    ...
    _removeFromGroup: function _removeFromGroup(info) {
      if (this.config.smartActionGroups) {
    ...
    _updateSmartActionTablesDefiendForCol: function _updateSmartActionTablesDefiendForCol(groupName) {
      if (this._smartActionsTable) {
    ...
    _removeGroupNameFromLayerFields: function _removeGroupNameFromLayerFields(info) {
      var fieldValidations,
          fieldValidationsArray = [],
          layerId;
      layerId = info.layerId;
      //get all possible instances for field validation of this layer id
      fieldValidationsArray = this._getLayersFieldValidations(fieldValidationsArray, this._configInfos, layerId);
            ...
            for (var field in fieldValidations) {
              if (fieldValidations && fieldValidations[field] && field === info.fieldName) {
                array.some(fieldValidations[field], function (existingValidation) {
                  if (existingValidation && info.action === existingValidation.actionName && existingValidation.filter && existingValidation.filter.hasOwnProperty('smartActionGroupName') && existingValidation.filter.smartActionGroupName === info.groupName) {
    ...
    _getLayersFieldValidations: function _getLayersFieldValidations(fieldValidations, configInfos, layerId) {
    ...
    _configureGroupSmartAction: function _configureGroupSmartAction(tr, isDelete) {
      var params = {
        nls: this.nls,
        map: this.map,
        filterInfo: { "filter": '' },
    ...
    _addSmartActionGroupRow: function _addSmartActionGroupRow(groupInfo) {
     ...
     */
    _updateSmartActionTablesDefiendForColForAllRows: function _updateSmartActionTablesDefiendForColForAllRows() {
      if (this._smartActionsTable) {
        var allRows = this._smartActionsTable.getRows();
    ...
    _addDefinedFor: function _addDefinedFor(priorityDetails, row) {
    ...
    _checkPriorityStatus: function _checkPriorityStatus(groupInfo) {
    ...
    _getFieldValidationsFromGroup: function _getFieldValidationsFromGroup(groupInfo, layerId, existingFieldValidations) {
      var fieldValidations = {};
      for (var field in groupInfo.appliedOn[layerId]) {
        ...
        if (groupInfo.appliedOn[layerId][field].Hide || groupInfo.appliedOn[layerId][field].Required || groupInfo.appliedOn[layerId][field].Disabled) {
          if (!actionsArray) {
            actionsArray = [{
              'actionName': "Hide"
            }, {
    ...
    _getSmartActionsForRelation: function _getSmartActionsForRelation(relatedLayerId) {
      var fieldValidations;
      if (this.config.smartActionGroups) {
        for (var groupName in this.config.smartActionGroups) {
          if (this.config.smartActionGroups[groupName].appliedOn.hasOwnProperty(relatedLayerId)) {
            fieldValidations = this._getFieldValidationsFromGroup(this.config.smartActionGroups[groupName], relatedLayerId, fieldValidations);
          }
        }
      }
      return fieldValidations;
    ...
    _getFieldValuesFromGroup: function _getFieldValuesFromGroup(groupInfo, layerId, existingFieldValidations) {
      var fieldValidations = {};
      for (var field in groupInfo.appliedOn[layerId]) {
        var actionsArray;
        if (existingFieldValidations && existingFieldValidations[field]) {
    ...
    _getAttributeActionsForRelation: function _getAttributeActionsForRelation(relatedLayerId) {
      var fieldValues;
      if (this.config.attributeActionGroups && this.config.attributeActionGroups.Preset) {
        for (var groupName in this.config.attributeActionGroups.Preset) {
          if (this.config.attributeActionGroups.Preset[groupName].appliedOn.hasOwnProperty(relatedLayerId)) {
            fieldValues = this._getFieldValuesFromGroup(this.config.attributeActionGroups.Preset[groupName], relatedLayerId, fieldValues);
          }
        }
      }
      return fieldValues;
    ...
    _createLayerChooserFromMap: function _createLayerChooserFromMap() {
    ...
    _createLayerChooserMapArgs: function _createLayerChooserMapArgs() {
      var layerChooserFromMapArgs;
    ...
    _isLayerEditable: function _isLayerEditable(currentLayer) {
      var editCapabilites,
          isEditable = false;
      if (currentLayer && currentLayer.layerObject) {
        editCapabilites = currentLayer.layerObject.getEditCapabilities();
        if (editCapabilites.canCreate || editCapabilites.canUpdate || editCapabilites.canDelete || editCapabilites.canUpdateGeometry) {
          isEditable = true;
        }
      }
      return isEditable;
    ...
    _getAllFilteredLayers: function _getAllFilteredLayers(layerInfosArray, defList) {
      array.forEach(layerInfosArray, lang.hitch(this, function (currentLayer) {
        ...
        if (!currentLayer.isLeaf()) {
          ...
          this._layerChooserFromMap.filter(currentLayer).then(lang.hitch(this, function (isValid) {
    ...
    _onDeleteAttributeActionsClick: function _onDeleteAttributeActionsClick(tr, actionName) {
      var fieldsPopup = new Popup({
     ...
     */
    _initAttributeActionsTable: function _initAttributeActionsTable(tableContainerNode) {
    ...
    _getAllLayersFieldValues: function _getAllLayersFieldValues(fieldValues, configInfos, layerId) {
      array.forEach(configInfos, function (layer) {
        if (layer.featureLayer && layer.featureLayer.id === layerId) {
        ...
        if (layer.relationshipInfos) {
          fieldValues = this._getAllLayersFieldValues(fieldValues, layer.relationshipInfos, layerId);
        }
      }, this);
      return fieldValues;
    },

    _removeAttributeGroupNameFromLayerFields: function _removeAttributeGroupNameFromLayerFields(info) {
      var layersFieldValues,
          allFieldValues = [],
          layerId;
      layerId = info.layerId;
            ...
            for (var field in layersFieldValues) {
              if (layersFieldValues && layersFieldValues[field] && field === info.fieldName) {
                ...
                for (var i = 0; i < fieldValues.length; i++) {
                  if (fieldValues[i].actionName === info.action && fieldValues[i].hasOwnProperty('attributeActionGroupName') && fieldValues[i].attributeActionGroupName === info.groupName) {
    ...
    _removeFromAttributeActionGroup: function _removeFromAttributeActionGroup(info) {
      if (this.config.attributeActionGroups) {
        if (this.config.attributeActionGroups[info.action]) {
          ...
          if (groupInfo.appliedOn[info.layerId] && groupInfo.appliedOn[info.layerId].indexOf(info.fieldName) > -1) {
     ...
     */
    _initAllAttributeActions: function _initAllAttributeActions() {
      //Create table for intersection action group
      this._intersectionActionGroupTable = this._initAttributeActionsTable(this.intersectionActionsTableNode);
      this.own(on(this._intersectionActionGroupTable, 'actions-edit', lang.hitch(this, this._configureIntersectionActionGroup)));
      this._intersectionActionGroupTable.onBeforeRowDelete = lang.hitch(this, function (tr) {
        this._onDeleteAttributeActionsClick(tr, "Intersection");
      });

      //Create table for address action group
      this._addressActionGroupTable = this._initAttributeActionsTable(this.addressActionsTableNode);
      this.own(on(this._addressActionGroupTable, 'actions-edit', lang.hitch(this, this._configureAddressActionGroup)));
      this._addressActionGroupTable.onBeforeRowDelete = lang.hitch(this, function (tr) {
        this._onDeleteAttributeActionsClick(tr, "Address");
      });

      //Create table for coordinates action group
      this._coordinatesActionGroupTable = this._initAttributeActionsTable(this.coordinatesActionsTableNode);
      this.own(on(this._coordinatesActionGroupTable, 'actions-edit', lang.hitch(this, this._configureCoordinatesActionGroup)));
      this._coordinatesActionGroupTable.onBeforeRowDelete = lang.hitch(this, function (tr) {
        this._onDeleteAttributeActionsClick(tr, "Coordinates");
      });

      //Create table for preset action group
      this._presetActionGroupTable = this._initAttributeActionsTable(this.presetActionsTableNode);
      this.own(on(this._presetActionGroupTable, 'actions-edit', lang.hitch(this, this._configurePresetActionGroup)));
      this._presetActionGroupTable.onBeforeRowDelete = lang.hitch(this, function (tr) {
    ...
    _addAttributeActionGroupRow: function _addAttributeActionGroupRow(groupInfo, actionName) {
    ...
    _populateAttributeActionGroups: function _populateAttributeActionGroups() {
    ...
    _getAttributeActionGroupConfig: function _getAttributeActionGroupConfig() {
      var attributeActionGroups = {
        "Intersection": this._getAttributeGroupsForAction(this._intersectionActionGroupTable),
        "Address": this._getAttributeGroupsForAction(this._addressActionGroupTable),
        "Coordinates": this._getAttributeGroupsForAction(this._coordinatesActionGroupTable),
        "Preset": this._getAttributeGroupsForAction(this._presetActionGroupTable)
      };
      return attributeActionGroups;
    },

    _getAttributeGroupsForAction: function _getAttributeGroupsForAction(table) {
      var rows,
          attributeGroupsForAction = {};
      if (table) {
        rows = table.getRows();
        if (rows && rows.length > 0) {
          array.forEach(rows, function (row) {
     ...
     */
    _configureIntersectionActionGroup: function _configureIntersectionActionGroup(tr, isDelete) {
      var params;
      params = {
        nls: this.nls,
        map: this.map,
    ...
    _onIntersectionBtnClick: function _onIntersectionBtnClick() {
      this._configureIntersectionActionGroup();
    },

    _configureAddressActionGroup: function _configureAddressActionGroup(tr, isDelete) {
      var params;
      params = {
        nls: this.nls,
        map: this.map,
     ...
     */
    _onAddressBtnClick: function _onAddressBtnClick() {
    ...
    _configureCoordinatesActionGroup: function _configureCoordinatesActionGroup(tr, isDelete) {
      var params;
      params = {
        nls: this.nls,
        map: this.map,
     ...
     */
    _onCoordinateBtnClick: function _onCoordinateBtnClick() {
    ...
    _configurePresetActionGroup: function _configurePresetActionGroup(tr, isDelete) {
      var params;
      params = {
        nls: this.nls,
        map: this.map,
     ...
     * This function is used to listen click event of addNew  preset AttributeActionButton.
     */
    _onPresetBtnClick: function _onPresetBtnClick() {
     ...
     */
    _filterAttributeDropDownOptions: function _filterAttributeDropDownOptions(currentRow) {
      var rows,
          configuredDataType,
          coordinatesOption = {};
    ...
    onTableEditClick: function onTableEditClick(table, row) {
      //table.finishEditing();
      var data = table.getRowData(row);
      table.editRow(row, data);
    },

    onRowAddClick: function onRowAddClick(table, data) {
      //table.finishEditing();
