  ...
  return declare([BaseWidgetSetting, Evented, _TemplatedMixin], {
    ...
    _initComponents: function _initComponents() {
      ...
      this.groupNameTextBox.validator = lang.hitch(this, function (value) {
        if (!value) {
          this.groupNameTextBox.set("invalidMessage", this.nls.smartActionsPage.requiredGroupNameMsg);
          return false;
        }
        if (value !== this.prevName && this.existingGroupNames.indexOf(value) > -1) {
          this.groupNameTextBox.set("invalidMessage", this.nls.smartActionsPage.uniqueGroupNameMsg);
          return false;
        }
        return true;
      });
    ...
    _addLayersDropDown: function _addLayersDropDown() {
      var layerChooserFromMapArgs, layerSelector, layerChooserFromMap, selectedLayerInfo, selectedLayer;
      ...
      layerSelector = new LayerChooserFromMapWithDropbox({ layerChooser: layerChooserFromMap });
     ...
     */
    _validateGroup: function _validateGroup() {
      var selectedLayer;
    ...
    _getTableData: function _getTableData() {
      var fields = [],
          layerInfo;
    ...
    _showFilter: function _showFilter(tr) {
      //Validate for valid groupName
      if (!this.groupNameTextBox.isValid()) {
        this.groupNameTextBox.focus();
        return false;
    ...
    onFilterInfoChanged: function onFilterInfoChanged(filterInfo) {
    ...
    _createHeaderObj: function _createHeaderObj(header, tr, index) {
      var headerIconDiv;
    ...
    _createHeaders: function _createHeaders(table) {
    ...
    _createPriorityIcons: function _createPriorityIcons(fieldRow, layerFieldDetails) {
    ...
    _createPriorityTable: function _createPriorityTable(selectedNode) {
      var fields2, args2;
      fields2 = [{
        name: 'priorityName',
    ...
    _getExistingPriority: function _getExistingPriority(selectedNode) {
    ...
    _populatePriorityTable: function _populatePriorityTable(selectedNode) {
    ...
    _fetchPriority: function _fetchPriority() {
    ...
    _changePriority: function _changePriority(priorityArr, selectedNode) {
      array.forEach(priorityArr, lang.hitch(this, function (prioritySelected, priorityIndex) {
        array.forEach(selectedNode.childNodes, lang.hitch(this, function (node, index) {
          if (priorityIndex === index) {
            ...
            domAttr.set(node, "title", this.nls.actionPage.actions[prioritySelected.toLowerCase()]);
          }
        }));
      }));
    },

    _createPriorityPopup: function _createPriorityPopup(selectedNode, layerFieldDetails) {
      var fieldsPopup = new Popup({
    ...
    _createLayerName: function _createLayerName(layerRow, layerId) {
    ...
    _createFieldNameTd: function _createFieldNameTd(fieldRow, fieldLabel) {
    ...
    _addRows: function _addRows(table, details) {
      ...
      for (var layerId in layerDetails) {
        ...
        for (var field in layerDetail) {
          ...
          // hide td
          this._createCheckBox(fieldRow, checkboxInfo, layerDetail[field].Hide, haveExistingExpressions);
          ...
          // required td
          this._createCheckBox(fieldRow, checkboxInfo, layerDetail[field].Required, haveExistingExpressions);
          ...
          // disabled td
          this._createCheckBox(fieldRow, checkboxInfo, layerDetail[field].Disabled, haveExistingExpressions);
          //set layers to be expanded if any of the action is enabled
          if (layerToExpand.indexOf(layerId) === -1 && (layerDetail[field].Hide || layerDetail[field].Required || layerDetail[field].Disabled)) {
    ...
    _createCheckBox: function _createCheckBox(fieldRow, checkBoxInfo, checked, haveExistingExpressions) {
    ...
    _createTable: function _createTable(appliedOn) {
    ...
    _validateIfLayerHaveUsedFields: function _validateIfLayerHaveUsedFields(allLayerFields, usedFieldsArray) {
      ...
      if (usedFieldsArray) {
        ...
        array.forEach(allLayerFields, function (field) {
          array.forEach(usedFieldsArray, function (usedField) {
            if (field.name === usedField.fieldObj.name && field.type === usedField.fieldObj.type) {
              usedFieldsLength--;
            }
          });
        }, this);
    ...
    _mergeActions: function _mergeActions(oldAction, newActions) {
      for (var i = 0; i < oldAction.length; i++) {
        for (var j = 0; j < newActions.length; j++) {
          if (newActions[j].filter && newActions[j].actionName === oldAction[i].actionName) {
    ...
    _mergeFieldValidations: function _mergeFieldValidations(fieldValidations) {
      ...
      array.forEach(fieldValidations, function (fieldValidation) {
        for (var field in fieldValidation) {
          if (mergedFieldValidation.hasOwnProperty(field)) {
            mergedFieldValidation[field] = this._mergeActions(mergedFieldValidation[field], fieldValidation[field]);
          } else {
    ...
    _getPartsInExpression: function _getPartsInExpression(allParts, partsArray) {
      if (partsArray) {
    ...
    _createLayerDetails: function _createLayerDetails(prevSettings) {
      ...
      array.forEach(this._totalLayers, function (layer) {
        ...
        if (!layer.isTable || layer.isTable && layer.layerObject.relationships.length > 0) {
          ...
          addLayer = this._validateIfLayerHaveUsedFields(configuredLayerInfo.allFields, parts);
          //if all fields in the expression are available or
          //if expression is not configured then only show the layer
          if (configuredLayerInfo && configuredLayerInfo.fieldInfos && (addLayer || !parts)) {
            ...
            fieldValidationsArray = this._getLayersFieldValidations(fieldValidationsArray, this._configInfos, layer.id);
            //merge all the field validations
            configuredLayerInfo.fieldValidations = this._mergeFieldValidations(fieldValidationsArray);
            ...
            array.forEach(configuredLayerInfo.fieldInfos, function (field) {
              ...
              if (field.type !== "esriFieldTypeGeometry" && field.type !== "esriFieldTypeOID" && field.type !== "esriFieldTypeBlob" && field.type !== "esriFieldTypeGlobalID" && field.type !== "esriFieldTypeRaster" && field.type !== "esriFieldTypeXML") {
                ...
                if (configuredLayerInfo.fieldValidations && configuredLayerInfo.fieldValidations[field.name]) {
                  prioritiesInfo = this._getPriorites(configuredLayerInfo.fieldValidations[field.name]);
                  layerDetails[layer.id][field.name].Priority = prioritiesInfo.Priority;
                  layerDetails[layer.id][field.name].ExistingExpressions = prioritiesInfo.ExistingExpressions;
                } else {
    ...
    _getPriorites: function _getPriorites(actions) {
      var priorites = [],
          haveExistingExpressions = {};
      array.forEach(actions, function (action) {
        priorites.push(action.actionName);
        if (action.filter && action.filter.displaySQL) {
          if (!action.filter.hasOwnProperty('smartActionGroupName') || !this.prevName || action.filter.hasOwnProperty('smartActionGroupName') && this.prevName && this.prevName !== action.filter.smartActionGroupName) {
    ...
    _getVaidationForAction: function _getVaidationForAction(allActions, actionName) {
    ...
    _applysettingsToField: function _applysettingsToField(layerId, groupInfo) {
          ...
          for (var field in allFields) {
            ...
            for (var j = 0; j < layerDetail.Priority.length; j++) {
              ...
              if (layerDetail[action]) {
                ...
                var existingValidation = this._getVaidationForAction(fieldValidations[field], action);
                ...
                if (existingValidation && existingValidation.filter && !layerDetail[action] && existingValidation.filter.hasOwnProperty('smartActionGroupName') && existingValidation.filter.smartActionGroupName === this.prevName) {
                  updatedActions = true;
    ...
    _removePrevSettingsFromLayerFields: function _removePrevSettingsFromLayerFields(layerId) {
            ...
            for (var field in fieldValidations) {
              if (fieldValidations && fieldValidations[field]) {
                array.forEach(fieldValidations[field], function (existingValidation) {
                  if (existingValidation && existingValidation.filter && existingValidation.filter.hasOwnProperty('smartActionGroupName') && existingValidation.filter.smartActionGroupName === this.prevName) {
     ...
     */
    _removeSettingsFromOtherGroups: function _removeSettingsFromOtherGroups(currentGroupName, layerId, field, action) {
      var groupsAppliedOn;
      ...
      if (this.existingGroups) {
        ...
        for (var groupName in this.existingGroups) {
          ...
          if (groupName !== currentGroupName && groupName !== this.prevName) {
            ...
            if (groupsAppliedOn && groupsAppliedOn.hasOwnProperty(layerId) && groupsAppliedOn[layerId].hasOwnProperty(field) && groupsAppliedOn[layerId][field].hasOwnProperty(action)) {
    ...
    _applySettingsInLayer: function _applySettingsInLayer(groupInfo) {
      for (var layerId in groupInfo.appliedOn) {
        var prevIndex;
        if (this._prevAppliedOnLayers && this._prevAppliedOnLayers.indexOf(layerId) > -1) {
