...
///////////////////////////////////////////////////////////////////////////
define(["dojo/_base/declare", "dojo/_base/lang", "dojo/_base/array", 'dojo/dom-construct', 'dojo/on', 'jimu/BaseWidgetSetting', 'dijit/_TemplatedMixin', 'jimu/dijit/CheckBox', 'jimu/utils', 'dojo/dom-attr', 'dojo/dom-class', 'dojo/query', 'dojo/string', 'dojo/text!./layerAndFieldsApplyOn.html'], function (declare, lang, array, domConstruct, on, BaseWidgetSetting, _TemplatedMixin, CheckBox, jimuUtils, domAttr, domClass, query, String, template) {
  return declare([BaseWidgetSetting, _TemplatedMixin], {
    ...
    _mergeFieldValuesForPreset: function _mergeFieldValuesForPreset(layerId) {
        ...
        for (var field in singleFieldValue) {
          ...
          if (!mergedFieldValues.hasOwnProperty(field)) {
            ...
            for (var j = 0; j < singleFieldValue[field].length; j++) {
              ...
              if (fieldValues.actionName === this.actionName && fieldValues.enabled && (!this.prevName || !fieldValues.hasOwnProperty('attributeActionGroupName') || fieldValues.hasOwnProperty('attributeActionGroupName') && fieldValues.attributeActionGroupName !== this.prevName)) {
     ...
     */
    _addLayerAndFields: function _addLayerAndFields() {
      var layerDetails,
          layerToExpand = [],
          layersFieldValues;
      ...
      for (var layerId in layerDetails) {
        if (Object.keys(layerDetails[layerId]).length > 0) {
          ...
          if (this.appliedOn && this.appliedOn.hasOwnProperty(layerId) && this.appliedOn[layerId].length > 0) {
          ...
          for (var field in layerDetails[layerId]) {
            ...
            if (layerDetails[layerId][field].editable) {
              ...
              if (layersFieldValues && layersFieldValues.hasOwnProperty(field)) {
                array.some(layersFieldValues[field], function (fieldValues) {
                  if (fieldValues.actionName === this.actionName && fieldValues.enabled && (!fieldValues.hasOwnProperty('attributeActionGroupName') || fieldValues.hasOwnProperty('attributeActionGroupName') && fieldValues.attributeActionGroupName !== this.prevName)) {
                    hasExistingFieldValues = true;
                    return true;
                  }
                }, this);
              }
              this._createFieldName(layerDetails[layerId][field], layerId, layerFieldDiv, hasExistingFieldValues);
            }
          }
        }
      }
      ...
      if (layerToExpand.length > 0) {
        setTimeout(lang.hitch(this, function () {
          ...
          array.forEach(layerToExpand, lang.hitch(this, function (rootLayerId) {
            ...
            if (rootLayerNode && rootLayerNode.length > 0 && domClass.contains(rootLayerNode[0], "esriCTToggleLayerExpanded")) {
     ...
     */
    _createLayerName: function _createLayerName(layerMainDiv, layerId) {
    ...
    _getLayersFieldValues: function _getLayersFieldValues(layerId) {
      var fieldValues;
     ...
     */
    _createFieldName: function _createFieldName(field, layerId, layerFieldDiv, hasExistingFieldValues) {
      var checkBox, fieldInfo;
      fieldInfo = jimuUtils.getDefaultPortalFieldInfo(field);
      var existingValueMsg = String.substitute(this.nls.attributeActionsPage.alreadyAppliedActionMsg, {
     ...
     */
    _parentNodeStateChanged: function _parentNodeStateChanged(evt) {
     ...
     */
    _childNodeStateChanged: function _childNodeStateChanged(evt) {
     ...
     */
    getCheckedFields: function getCheckedFields(groupInfo) {
      var appliedOn = {};
    ...
    _removeSettingsFromOtherGroups: function _removeSettingsFromOtherGroups(currentGroupName, layerId, field) {
      var groupsAppliedOn;
      ...
      if (this.existingGroups) {
        ...
        for (var groupName in this.existingGroups) {
          ...
          if (groupName !== currentGroupName && groupName !== this.prevName) {
            ...
            if (groupsAppliedOn && groupsAppliedOn.hasOwnProperty(layerId) && groupsAppliedOn[layerId].indexOf(field) > -1) {
          ...
          if (layersFieldValues) {
            for (var field in layersFieldValues) {
              ...
              for (var i = 0; i < fieldValues.length; i++) {
                if (fieldValues[i].actionName === this.actionName && fieldValues[i].hasOwnProperty('attributeActionGroupName') && fieldValues[i].attributeActionGroupName === this.prevName) {
    ...
    _applysettingsToField: function _applysettingsToField(layerId, allFields, groupInfo) {
          ...
          if (this.appliedOn && this.appliedOn.hasOwnProperty(layerId)) {
            ...
            if (checkedFields && checkedFields.length > 0) {
              array.forEach(checkedFields, function (field) {
                if (allFields.indexOf(field) === -1 && layersFieldValues.hasOwnProperty(field)) {
                  var fieldValues;
    ...
    _applySettingsInLayer: function _applySettingsInLayer(groupInfo, appliedOn) {
     ...
     */
    _applyPrevSettings: function _applyPrevSettings() {
      if (this.appliedOn) {
        for (var layerId in this.appliedOn) {
          if (this.appliedOn.hasOwnProperty(layerId)) {
            ...
            if (checkedFields && checkedFields.length > 0) {
              ...
              array.forEach(this.checkBoxNodes[layerId], lang.hitch(this, function (checkBox) {
