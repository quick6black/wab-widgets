...
define(["dojo/_base/declare", "dojo/_base/lang", "dojo/Deferred", "jimu/BaseWidgetSetting", "esri/tasks/query", "esri/tasks/QueryTask", "esri/geometry/Extent", "esri/geometry/geometryEngine", "esri/SpatialReference", "esri/geometry/webMercatorUtils", "dojo/_base/array"], function (declare, lang, Deferred, BaseWidgetSetting, Query, QueryTask, Extent, geometryEngine, SpatialReference, webMercatorUtils, array) {
  return declare([BaseWidgetSetting], {
    ...
    _canGetGeodesicBuffers: function _canGetGeodesicBuffers(geometry) {
      var outSR;
    ...
    _getbufferedGeometry: function _getbufferedGeometry(featureGeometry, bufferDistance) {
      var bufferedGeometries;
      if (this._canGetGeodesicBuffers(featureGeometry)) {
        bufferedGeometries = geometryEngine.geodesicBuffer([featureGeometry], [bufferDistance], "meters", true);
      } else {
        bufferedGeometries = geometryEngine.buffer([featureGeometry], [bufferDistance], "meters", true);
      }
    ...
    getToleranceValueInMeters: function getToleranceValueInMeters(toleranceSettings) {
    ...
    setMaxToleranceForLayer: function setMaxToleranceForLayer(layerId, toleranceSettings) {
      var valueInMeters;
      ...
      if (valueInMeters > 0 && (!this.maxToleranceForlayer.hasOwnProperty(layerId) || this.maxToleranceForlayer[layerId] < valueInMeters)) {
        this.maxToleranceForlayer[layerId] = valueInMeters;
      }
    },

    convertToMeters: function convertToMeters(length, inputUnit) {
    ...
    getDistinctLayers: function getDistinctLayers(selectedLayerInfo, featureGeometry) {
          ...
          for (var i = 0; i < selectedLayerInfo.fieldValues[fieldName].length; i++) {
            ...
            if (copyAction.actionName === "Intersection" && copyAction.enabled) {
              ...
              for (var j = 0; j < copyAction.fields.length; j++) {
                ...
                if (this.layersInvolvedInIgnoreLayerRanking.indexOf(field.layerId) === -1 && copyAction.ignoreLayerRanking) {
                ...
                if (copyAction.ignoreLayerRanking) {
                  ...
                  for (var j = 0; j < copyAction.fields.length; j++) {
                    ...
                    if (!result[fieldName].hasOwnProperty(layerId)) {
                      featureAttributes = this._getProcessedFeature(layerId, field.toleranceSettings);
    ...
    _getIntersectingFeatures: function _getIntersectingFeatures(layerId, toleranceSettings) {
      ...
      if (this.completeFeatures.hasOwnProperty(layerId)) {
        ...
        if (allFeatures.length > 0) {
          ...
          if (toleranceValueInMeters) {
            bufferedGeometry = this._getbufferedGeometry(this.drawnGeometry, toleranceValueInMeters);
          } else {
            ...
            if (!this.maxToleranceForlayer.hasOwnProperty(layerId) || !this.maxToleranceForlayer[layerId]) {
              intersectingFeatures = allFeatures;
            } else {
              if (this.drawnGeometry.type === "point") {
                bufferedGeometry = this.pointToExtent(this.map, this.drawnGeometry, 20);
              } else {
          ...
          if (bufferedGeometry) {
            for (i = 0; i < allFeatures.length; i++) {
              ...
              if (feature.geometry && geometryEngine.intersects(bufferedGeometry, feature.geometry)) {
    ...
    _getClosestFeature: function _getClosestFeature(fields) {
      var returnValue,
          featureAttributes,
          layerObject,
          layerId,
      ...
      for (j = 0; j < fields.length; j++) {
        ...
        intersectingFeatures = this._getIntersectingFeatures(layerId, fields[j].toleranceSettings);
        ...
        for (i = 0; i < intersectingFeatures.length; i++) {
          ...
          if (this.drawnGeometry && intersectingFeatures[i].geometry) {
            intersectingFeatures[i].distanceToLocation = geometryEngine.distance(this.drawnGeometry, intersectingFeatures[i].geometry, "meters");
          }
      ...
      if (allIntersectingFeatures.length > 0) {
        ...
        layerObject = this._jimuLayerInfos.getLayerOrTableInfoById(allIntersectingFeatures[0].layerId).layerObject;
    ...
    _getProcessedFeature: function _getProcessedFeature(layerId, toleranceSettings) {
      var layerObject, featureAttributes, intersectingFeatures;
      intersectingFeatures = [];
      //get intersecting features accroding to current tolerance settings
      intersectingFeatures = this._getIntersectingFeatures(layerId, toleranceSettings);
    ...
    _getIntersectionsForEachField: function _getIntersectionsForEachField(distinctLayers, featureGeometry, completeDef) {
    ...
    _filterQueriedLayers: function _filterQueriedLayers(distinctLayers) {
      var layerId;
    ...
    _asyncIntersectionRequests: function _asyncIntersectionRequests(distinctLayers, geometry, fieldDef) {
      if (!fieldDef) {
    ...
    _syncIntersectionRequests: function _syncIntersectionRequests(distinctLayers, geometry, fieldDef) {
      if (!fieldDef) {
        fieldDef = new Deferred();
      }
      distinctLayers = this._filterQueriedLayers(distinctLayers);
    ...
    _intersectionResult: function _intersectionResult(result, fieldDef) {
    ...
    pointToExtent: function pointToExtent(map, point, toleranceInPixel) {
    ...
    _getAppropriateGeometryForQuery: function _getAppropriateGeometryForQuery(layerId, featureGeometry) {
      var returGeometry;
      //if tolreance value is defiend for this layer use it otherwise
      //use the geometry as it is for geometries other than point
      //and for point calculate the point to extent of 20px tolerance
      if (this.maxToleranceForlayer.hasOwnProperty(layerId)) {
        returGeometry = this._getbufferedGeometry(featureGeometry, this.maxToleranceForlayer[layerId]);
      } else {
    ...
    _getIntersectedFeatures: function _getIntersectedFeatures(layerId, featureGeometry) {
      ...
      if (this.layersInvolvedInIgnoreLayerRanking.indexOf(layerId) > -1 || this.maxToleranceForlayer.hasOwnProperty(layerId)) {
        query.returnGeometry = true;
        query.outSpatialReference = new SpatialReference(featureGeometry.spatialReference.toJson());
      } else {
