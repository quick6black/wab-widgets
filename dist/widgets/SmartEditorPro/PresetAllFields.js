...
///////////////////////////////////////////////////////////////////////////
define(["dojo/_base/declare", "dojo/Evented", "dojo/_base/lang", "dojo/_base/array", "dojo/dom-attr", 'dojo/dom-construct', 'dojo/on', "./utils", './presetUtils', 'dijit/_TemplatedMixin', 'jimu/BaseWidgetSetting', "jimu/dijit/Popup"], function (declare, Evented, lang, array, domAttr, domConstruct, on, editUtils, presetUtils, _TemplatedMixin, BaseWidgetSetting, Popup) {
  return declare([BaseWidgetSetting, Evented, _TemplatedMixin], {
    ...
    postCreate: function postCreate() {
      ...
      this.removeFromGroupCollection = {};
    ...
    _getRelatedLayersInfo: function _getRelatedLayersInfo(relations, relatedLayersInfo) {
      var newRelations = [];
      for (var i = 0; i < relations.length; i++) {
        var relatedInfo = {};
        for (var key in relations[i]) {
          if (key === 'featureLayer' && relations[i][key].id) {
            relatedInfo[key] = {
              id: relations[i][key].id
            };
          }
    ...
    _getPresetFieldsForLayers: function _getPresetFieldsForLayers(allLayersInfo) {
      var presetFields = [];
            ...
            for (var j = 0; j < currentConfig.fieldValues[fieldName].length; j++) {
              ...
              if (copyAction.actionName === "Preset" && copyAction.enabled) {
                if (!this.actionsCollection[fieldName]) {
                  ...
                  this.removeFromGroupCollection[fieldName] = [];
                }
                this.actionsCollection[fieldName].push(copyAction);
                if (copyAction.hasOwnProperty("attributeActionGroupName")) {
                  removeFromGroupInfo.groupName = copyAction.attributeActionGroupName;
                }
                this.removeFromGroupCollection[fieldName].push(removeFromGroupInfo);
          ...
          if (presetFields.length > 0) {
            for (var k = 0; k < presetFields.length; k++) {
              ...
              if (this.presetFieldInfos.hasOwnProperty(fName)) {
                this.presetFieldInfos[fName] = presetUtils.changeFieldToMostRestrictive(this.presetFieldInfos[fName], fieldInfo);
              } else {
    ...
    _createPresetFieldInfos: function _createPresetFieldInfos() {
      this.presetFieldInfos = {};
    ...
    _disableActionsForDeletedFields: function _disableActionsForDeletedFields() {
      array.forEach(this._deletedFields, lang.hitch(this, function (fieldName) {
        ...
        for (var i = 0; i < this.actionsCollection[fieldName].length; i++) {
          //delete attribute action group name as we are disabling the preset action
          if (this.removeFromGroupCollection[fieldName][i].hasOwnProperty("groupName")) {
            var removeGroupInfo = this.removeFromGroupCollection[fieldName][i];
            delete this.actionsCollection[fieldName][i].attributeActionGroupName;
            this.removeFromGroup(removeGroupInfo);
          }
          lang.mixin(this.actionsCollection[fieldName][i], { "enabled": false });
        }
      }));
    },
    ...
    _deleteField: function _deleteField(evt) {
      var fieldName;
    ...
    createContent: function createContent() {
      ...
      for (var fieldName in this.presetFieldInfos) {
        ...
        var fieldNode = domConstruct.create("div", {}, this.presetContainer);
        //Set the field label in preset popup
        domConstruct.create("div", {
    ...
    _savePresetValues: function _savePresetValues() {
      var isValid = true,
          presetInfos = {};
