...
define(["dojo/_base/declare", "dojo/_base/lang", "dojo/_base/array", 'dojo/_base/html', "dojo/on", "dojo/query", 'dojo/Deferred', "dijit/_WidgetsInTemplateMixin", "dijit/registry", "jimu/BaseWidgetSetting", 'jimu/dijit/ImageChooser', "dojo/text!./Edit.html", "jimu/dijit/ServiceURLInput", "jimu/SpatialReference/wkidUtils", 'jimu/utils', "../utils"], function (declare, lang, array, html, on, query, Deferred, _WidgetsInTemplateMixin, registry, BaseWidgetSetting, ImageChooser, template, ServiceURLInput, SRUtils, jimuUtils, utils) {
  return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
    ...
    postCreate: function postCreate() {
      ...
      this.url.setProcessFunction(lang.hitch(this, '_onServiceFetch', this.url), lang.hitch(this, '_onServiceFetchError'));
    },
    ...
    startup: function startup() {
      ...
      if (this.basemap && this.basemap.thumbnailUrl) {
        if (reg.test(this.basemap.thumbnailUrl)) {
          ...
          thumbnailUrl = jimuUtils.processUrlInWidgetConfig(this.basemap.thumbnailUrl, this.folderUrl);
        }
      } else {
    ...
    _checkTitle: function _checkTitle(title) {
      var validTitle = true;
    ...
    _onBaseMapTitleChange: function _onBaseMapTitleChange(title) {
    ...
    _checkProceed: function _checkProceed(errorMessage) {
      var canProceed = true;
      var urlDijits = [];
    ...
    _onServiceFetch: function _onServiceFetch(urlDijit, evt) {
      ...
      if (this._isStringEndWith(url, '/MapServer') || this._isStringEndWith(url, '/ImageServer')) {
        var curMapSpatialRefObj = this.map.spatialReference;
        var basemapSpatialRefObj = evt.data.spatialReference || evt.data.extent.spatialReference || evt.data.initialExtent.spatialReference || evt.data.fullExtent.spatialReference;
        if (curMapSpatialRefObj && basemapSpatialRefObj && SRUtils.isSameSR(curMapSpatialRefObj.wkid, basemapSpatialRefObj.wkid) && utils.tilingSchemeCompatible(this.map.__tileInfo, evt.data.tileInfo, wider)) {
    ...
    _isStringEndWith: function _isStringEndWith(s, endS) {
      return s.lastIndexOf(endS) + endS.length === s.length;
    },

    _onServiceFetchError: function _onServiceFetchError() {},

    onAddLayerUrl: function onAddLayerUrl() {
      this.addLayerUrl();
    },

    addLayerUrl: function addLayerUrl(url) {
      /*jshint unused: false*/
      ...
      urlInput.setProcessFunction(lang.hitch(this, '_onServiceFetch', urlInput), lang.hitch(this, '_onServiceFetchError'));

      this._checkProceed();
    },

    _onDeleteClick: function _onDeleteClick(urlDom) {
      html.destroy(urlDom);
      this._checkProceed();
    },

    _getUrlDijits: function _getUrlDijits() {
      var urlDijits = [];
    ...
    _getEditedBaseMap: function _getEditedBaseMap() {
      ...
      var basemap = {
        ...
        layers: [],
        spatialReference: this.basemap && this.basemap.spatialReference || this.map.spatialReference
      };
      ...
      if (reg.test(this.imageChooser.imageData)) {
        ...
        jimuUtils.getBase64Data(utils.getStanderdUrl(this.imageChooser.imageData)).then(lang.hitch(this, function (data) {
          if (data) {
    ...
    _onEditOk: function _onEditOk(settingWidget) {
      if (this.basemap) {
    ...
    _onEditClose: function _onEditClose(settingWidget) {
