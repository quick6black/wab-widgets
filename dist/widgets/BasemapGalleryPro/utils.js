...
define(['dojo/_base/lang', 'dojo/Deferred', 'dojo/json', 'dojo/_base/array', 'dojo/promise/all', 'esri/SpatialReference', 'jimu/portalUtils', 'jimu/shared/basePortalUrlUtils', 'esri/request', 'dojo/text!./esri_tileinfo.json'], function (lang, Deferred, JSON, array, all, SpatialReference, portalUtils, basePortalUrlUtils, esriRequest, esriTileInfoText) {
    ...
    getWebMapsFromBasemapGalleryGroup(portalUrl).then(function (response) {
      ...
      array.forEach(basemapItems, function (basemapItem) {
        ...
        basemapItem.getItemData().then(function (basemapItemData) {
          mo._getBasemapSpatialReference(basemapItem, basemapItemData).then(lang.hitch(this, function (basemapSpatialRef) {
            var basemapLayers = basemapItemData.baseMap.baseMapLayers;
            mo.isBasemapCompatibleWithMap(basemapSpatialRef, basemapLayers, map).then(lang.hitch(this, function (compatible) {
              if (compatible) {
                def.resolve({
  ...
  mo.isBasemapCompatibleWithMap = function (basemapSR, basemapLayers, map) {
    ...
    if (!mapSR || !basemapLayers || basemapLayers.length <= 0 || !basemapSR || !mapSR.equals(new SpatialReference(+basemapSR.wkid))) {
      def.resolve(false);
      return def;
    }

    // 2. check whether tiling schemes are compatible
    if (map.getNumLevels() === 0) {
      // current map is dynamic
      if (basemapLayers[0].layerType === "OpenStreetMap" || basemapLayers[0].layerType && basemapLayers[0].layerType.indexOf("BingMaps") > -1 || basemapLayers[0].layerType === "WebTiledLayer" || basemapLayers[0].layerType === "VectorTileLayer" || basemapLayers[0].layerType === "ArcGISImageServiceVectorLayer" || basemapLayers[0].layerType === 'ArcGISTiledImageServiceLayer') {
        def.resolve(false);
      } else {
        def.resolve(true);
      }
    } else {
      // current map is tiled map service
      if (basemapLayers[0].layerType && basemapLayers[0].layerType.indexOf('ArcGIS') === 0 && basemapLayers[0].url) {
        ...
        getServiceInfo(basemapLayers[0].url).then(function (res) {
          ...
          } else if (res && res.capabilities && (res.capabilities.indexOf('Map') >= 0 || res.capabilities.indexOf('Image') >= 0)) {
            // dynamic map/image service
            def.resolve(true);
          } else if (res && res.error && res.error.code === 499) {
            // secure service, token required
            def.resolve(true);
          } else {
            // other error, i.e. service not available
            def.resolve(false);
          }
        });
      } else if (basemapLayers[0].layerType === 'WMS') {
        // wms layer
        def.resolve(true);
        ...
        // WebTiledLayer, WMTSLayer and other tiled map service layer
  ...
  mo.isSameBasemapLayer = function (layer1, layer2) {
    ...
    if (layer1.layerType && layer2.layerType) {
      ...
      // ArcGIS web service
      if (layer1.layerType === 'ArcGISImageServiceVectorLayer' || layer1.layerType === 'ArcGISTiledImageServiceLayer' || layer1.layerType === 'ArcGISTiledMapServiceLayer' || layer1.layerType === 'ArcGISMapServiceLayer' || layer1.layerType === 'ArcGISImageServiceLayer') {
        url1 = _getStanderdUrl(layer1.url || "");
        url2 = _getStanderdUrl(layer2.url || "");
        return url1.toLowerCase() === url2.toLowerCase();
      }
      if (layer1.layerType === 'BingMapsAerial' || layer1.layerType === 'BingMapsRoad' || layer1.layerType === 'BingMapsHybrid' || layer1.layerType === 'OpenStreetMap') {
  ...
  mo.isBingMap = function (basemap) {
    ...
    for (var i = 0; i < basemap.layers.length; i++) {
      if (basemap.layers[i].type === "BingMapsAerial" || basemap.layers[i].type === "BingMapsRoad" || basemap.layers[i].type === "BingMapsHybrid") {
        return true;
      }
    }
    return false;
  ...
  mo.isNoUrlLayerMap = function (basemapLayers) {
    for (var i = 0; i < basemapLayers.length; i++) {
      if (basemapLayers[i].type === "BingMapsAerial" || basemapLayers[i].type === "BingMapsRoad" || basemapLayers[i].type === "BingMapsHybrid" || basemapLayers[i].type === "OpenStreetMap") {
        return true;
      }
    }
    return false;
  ...
  mo.getBasemapGalleryGroup = function (portalUrl, groupQueryString) {
    ...
    if (ownerIndex >= 0) {
      ...
      if (area === 'uk' && groupQueryString.indexOf('esri_ukr') < 0 || area === 'id' && groupQueryString.indexOf('esri_ind') < 0) {
  ...
  mo._getBasemapSpatialReference = function (basemapItem, basemapItemData) {
    ...
    if (basemapItem.owner && basemapItem.owner.indexOf("esri_") === 0 || mo.isNoUrlLayerMap(basemapItemData.baseMap.baseMapLayers)) {
      basemapSpatialRef = {
        wkid: "102100"
      };
      ...
      if (lyr.url && lyr.url.indexOf('rest/services') > 0) {
        async = true;
        getServiceInfo(basemapItemData.baseMap.baseMapLayers[0].url).then(lang.hitch(this, function (res) {
