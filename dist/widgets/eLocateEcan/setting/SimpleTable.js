...
define(['dojo/_base/declare', 'dijit/_WidgetBase', 'dijit/_TemplatedMixin', 'dojo/Evented', 'dojo/_base/lang', 'dojo/_base/html', 'dojo/_base/array', 'dojo/on', 'dojo/query', 'jimu/utils'], function (declare, _WidgetBase, _TemplatedMixin, Evented, lang, html, array, on, query, jimuUtils) {
  return declare([_WidgetBase, _TemplatedMixin, Evented], {
    baseClass: 'jimu-simple-table',
    declaredClass: 'jimu.dijit.SimpleTable',
    templateString: '<div>' + '<div class="head-section" data-dojo-attach-point="headDiv"></div>' + '<div class="body-section" data-dojo-attach-point="bodyDiv">' + '<div class="table-div" data-dojo-attach-point="tableDiv">' + '<table class="table" cellspacing="0" onselectstart="return false;">' + '<colgroup></colgroup>' + '<thead class="simple-table-thead simple-table-title"></thead>' + '<tbody class="simple-table-tbody"></tbody>' + '</table>' + '</div>' + '</div>' + '</div>',
    _name: null,
    _rowIndex: 0,
    _rowHeight: 30,
    _headHeight: 36,
    ...
    */
    ...
    _initSelf: function _initSelf() {
      ...
      this.own(jimuUtils.bindClickAndDblclickEvents(this.table, lang.hitch(this, function (evt) {
    ...
    _initAttachPoints: function _initAttachPoints() {
    ...
    addRows: function addRows(rowsData) {
      var results = [];
    ...
    addRow: function addRow(rowData, /* optional */dontUpdateUI, fieldInfo) {
      this._rowIndex++;
      var result = {
        success: false,
        tr: null,
      ...
      if (!this.fields || (typeof rowData === 'undefined' ? 'undefined' : _typeof(rowData)) !== 'object') {
        return result;
      }
    ...
    deleteRow: function deleteRow(tr) {
      if (tr) {
        html.destroy(tr);
        this._updateUI();
        this._onDeleteRow(tr);
    ...
    _updateRowClassName: function _updateRowClassName() {
    ...
    _createTextTd: function _createTextTd(tr, fieldMeta, fieldData) {
      var td = null;
    ...
    _createNormalTextTd: function _createNormalTextTd(tr, fieldMeta, fieldData) {
      var strTd = '<td class="simple-table-cell normal-text-td">' + '<div class="normal-text-div"></div></td>';
    ...
    _createEditableTextTd: function _createEditableTextTd(tr, fieldMeta, fieldData) {
    ...
    _createRadioTd: function _createRadioTd(tr, fieldMeta, fieldData) {
    ...
    _createCheckboxTd: function _createCheckboxTd(tr, fieldMeta, fieldData) {
    ...
    /*Added rowData*/
    _createActionsTd: function _createActionsTd(tr, fieldMeta, rowData) {
      var tdStr = '<td class="actions-td">' + '<div class="action-item-parent jimu-float-leading"></div></td>';
      var td = html.toDom(tdStr);
      html.addClass(td, 'simple-table-cell');
    ...
    _calculateActionsWidth: function _calculateActionsWidth(fieldMeta) {
    ...
    _createEmptyTd: function _createEmptyTd(tr, fieldMeta) {
    ...
    _createExtensionTd: function _createExtensionTd(tr, fieldMeta, fieldData) {
      var td = html.create('td', {
        'class': fieldMeta.name
      }, tr);
      html.addClass(td, 'simple-table-cell');
    ...
    editRow: function editRow(tr, rowData) {
      var result = {
        success: false,
        tr: null,
        errorCode: null,
    ...
    _editNormalText: function _editNormalText(td, fieldMeta, fieldData) {
      /*jshint unused: false*/
    ...
    _editEditableText: function _editEditableText(td, fieldMeta, fieldData) {
      /*jshint unused: false*/
      var editableDiv = query('div', td)[0];
      editableDiv.innerHTML = fieldData || "";
      var editableInput = query('input', td)[0];
    ...
    _editRadio: function _editRadio(td, fieldMeta, fieldData) {
      /*jshint unused: false*/
      var radio = query('input', td)[0];
      radio.checked = fieldData === true;
    },

    _editCheckbox: function _editCheckbox(td, fieldMeta, fieldData) {
      /*jshint unused: false*/
      var checkbox = query('input', td)[0];
      checkbox.checked = fieldData === true;
    },

    _editExtension: function _editExtension(td, fieldMeta, fieldData) {
      if (fieldMeta.setValue && typeof fieldMeta.setValue === 'function') {
        fieldMeta.setValue(td, fieldData);
      }
    },

    _getAllRows: function _getAllRows() {
      return query('.simple-table-row', this.tbody);
    },

    _getNotEmptyRows: function _getNotEmptyRows() {
      var trs = this._getAllRows();
    ...
    _getEmptyRows: function _getEmptyRows() {
      var trs = this._getAllRows();
      return array.filter(trs, lang.hitch(this, function (tr) {
        return html.hasClass(tr, 'empty');
      }));
    ...
    getData: function getData( /*optional*/ignoredTr) {
      var trs = this._getNotEmptyRows();
      if (ignoredTr) {
    ...
    _getRowDataByTr: function _getRowDataByTr(tr) {
      var rowData = null;
      if (tr) {
        rowData = {};
      } else {
    ...
    getRowDataArrayByFieldValue: function getRowDataArrayByFieldValue(fieldName, fieldValue, /*optional*/ignoredTr) {
      var result = [];
      if (!this.fields) {
        return [];
      }
    ...
    _onClickRow: function _onClickRow(tr) {
      this.emit('row-click', tr);
    },

    _onDblClickRow: function _onDblClickRow(tr) {
      this.emit('row-dblclick', tr);
    },

    _onSelectRow: function _onSelectRow(tr) {
      this.emit('row-select', tr);
    },

    _onAddRow: function _onAddRow(tr) {
      this.emit('row-add', tr);
    },

    _onEditRow: function _onEditRow(tr) {
      this.emit('row-edit', tr);
    },

    _onDeleteRow: function _onDeleteRow(tr) {
      this.emit('row-delete', tr);
    },

    _onEnterRow: function _onEnterRow(tr) {
      this.emit('row-enter', tr);
    },

    _onClearRows: function _onClearRows(trs) {
      this.emit('rows-clear', trs);
    },

    _onActionsEdit: function _onActionsEdit(tr) {
      this.emit('actions-edit', tr);
    },

    onBeforeRowUp: function onBeforeRowUp(tr) {
      /*jshint unused : false*/
      return true;
    },

    onBeforeRowDown: function onBeforeRowDown(tr) {
      /*jshint unused : false*/
      return true;
    },

    onBeforeRowEdit: function onBeforeRowEdit(tr) {
      /*jshint unused : false*/
      return true;
    },

    onBeforeRowDelete: function onBeforeRowDelete(tr) {
      /*jshint unused : false*/
      return true;
    }
  });
