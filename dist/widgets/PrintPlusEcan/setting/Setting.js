...
define(['dojo/_base/declare', 'jimu/BaseWidgetSetting', 'dijit/_WidgetsInTemplateMixin', "dojo/_base/lang", 'dojo/on', 'dojo/Deferred', "dojo/dom-style", "dojo/dom-attr", "esri/request", 'jimu/dijit/Message', 'jimu/portalUtils', 'jimu/portalUrlUtils', 'jimu/utils', "dojo/store/Memory", 'dijit/form/ValidationTextBox', 'dijit/form/ComboBox', 'jimu/dijit/CheckBox', 'dijit/form/SimpleTextarea'], function (declare, BaseWidgetSetting, _WidgetsInTemplateMixin, lang, on, Deferred, domStyle, domAttr, esriRequest, Message, portalUtils, portalUrlUtils, utils, Memory) {
  return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
    ...
    validator: function validator(value) {
      ...
      if (value === portalNewPrintUrl || /^https?:\/\/.+sharing\/tools\/newPrint$/.test(value) || /^https?:\/\/.+\/GPServer\//.test(value)) {
        return true;
      }
    ...
    _getPrintTaskInfo: function _getPrintTaskInfo(taskUrl) {
    ...
    _handlePrintInfo: function _handlePrintInfo(data) {
      ...
      var hasWebmapParam = false,
          hasOutputParam = false;

      if (data && data.parameters) {
        var len = data.parameters.length;
        var webmapParam = 'web_map_as_json',
            outputParam = 'output_file';
        for (var i = 0; i < len; i++) {
          ...
          if (param.name === "Format" || param.name === "Layout_Template") {
            ...
            if (param.name === "Format") {
              ...
              this.defaultFormat.set('store', this.memoryFormat);
              if (this.config.serviceURL === this.serviceURL.get('value') && this.config.defaultFormat) {
              ...
              this.defaultLayout.set('store', this.memoryLayout);
              if (this.config.serviceURL === this.serviceURL.get('value') && this.config.defaultLayout) {
    ...
    loadPrintURL: function loadPrintURL() {
      this._getPrintTaskURL(this.appConfig.portalUrl).then(lang.hitch(this, function (printServiceUrl) {
        this.serviceURL.set('value', printServiceUrl);
      }));
    },

    _getPrintTaskURL: function _getPrintTaskURL(portalUrl) {
      ...
      def.then(lang.hitch(this, function (response) {
        var printServiceUrl = response && response.helperServices && response.helperServices.printTask && response.helperServices.printTask.url;
        if (printServiceUrl) {
