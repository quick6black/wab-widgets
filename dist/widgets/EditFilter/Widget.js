...
'esri/urlUtils', 'esri/geometry/Point', 'jimu/WidgetManager', 'dijit/form/CheckBox'], function (declare, _WidgetsInTemplateMixin, BaseWidget, dijit, FilterParameters, dom, domConstruct, domClass, domAttr, domStyle, on, query, string, lang, array, locale, Select, TextBox, DateTextBox, NumberTextBox, registry, LayerInfos, utils, FilterManager, Query, QueryTask, geometryEngine, FeatureLayer, saveJson, readJson, LayersHandler, entities, esriUrlUtils, Point, WidgetManager, Checkbox) {
  ...
  return declare([BaseWidget, _WidgetsInTemplateMixin], {
    ...
    */
        ...
        if (operLayerInfos._layerInfos && operLayerInfos._layerInfos.length > 0) {
          ...
          array.forEach(this.layerList, lang.hitch(this, function (layer) {
            if (layer.originOperLayer.layerType !== "ArcGISTiledMapServiceLayer" && typeof layer.originOperLayer.featureCollection === 'undefined') {
              ...
              } else if (typeof layer.layerObject.defaultDefinitionExpression !== 'undefined' && typeof layer.layerObject.getDefinitionExpression() === 'function') {
                this.defaultDef.push({
                  layer: layer.id,
    ...
    _pullMapServiceFields: function _pullMapServiceFields(pLayerInfos) {
      this.msLayersDesc = [];
    ...
    checkDomainUse: function checkDomainUse(pParam) {
      this.useDomain = null;
      this.useValue = null;
    ...
    checkDateUse: function checkDateUse(pParam) {
      this.useDate = null;
      array.forEach(this.config.groups, lang.hitch(this, function (group) {
        if (group.name === pParam.group) {
          array.forEach(group.layers, lang.hitch(this, function (grpLayer) {
    ...
    createOperatorSelection: function createOperatorSelection(pCell, pValue) {
      var ObjList = [{ 'value': '=', 'label': this.nls.inputs.optionEQUAL }, { 'value': '<>', 'label': this.nls.inputs.optionNOTEQUAL }, { 'value': '>', 'label': this.nls.inputs.optionGREATERTHAN }, { 'value': '>=', 'label': this.nls.inputs.optionGREATERTHANEQUAL }, { 'value': '<', 'label': this.nls.inputs.optionLESSTHAN }, { 'value': '<=', 'label': this.nls.inputs.optionLESSTHANEQUAL }, { 'value': 'START', 'label': this.nls.inputs.optionSTART }, { 'value': 'END', 'label': this.nls.inputs.optionEND }, { 'value': 'LIKE', 'label': this.nls.inputs.optionLIKE }, { 'value': 'NOT LIKE', 'label': this.nls.inputs.optionNOTLIKE }, { 'value': 'd<=', 'label': this.nls.inputs.optionONORBEFORE }, { 'value': 'd>=', 'label': this.nls.inputs.optionONORAFTER }];
    ...
    createInputFilter: function createInputFilter(pCell, pValue) {
    ...
    createValueList: function createValueList(pValue, pDijit) {
      var filter = {};
      var parts = [];
      var holder = "";
      array.forEach(this.config.groups, lang.hitch(this, function (group) {
    ...
    _callFilterDijit: function _callFilterDijit(pGroupLayer, pValue, pUrl, pLayerObject, pParts, pFilter, pDijit) {
      var partsObj = {};
      partsObj.fieldObj = {};
      ...
      if (pGroupLayer.dataType.indexOf("Integer") > -1 || pGroupLayer.dataType.indexOf("Double") > -1 || pGroupLayer.dataType.indexOf("Short") > -1) {
        if (this.useDomain) {
    ...
    formatSpacing: function formatSpacing(pCell) {
      domStyle.set(pCell, { paddingLeft: "2px", paddingRight: "2px" });
    },

    createConditionSelection: function createConditionSelection(pCell, pValue) {
      domConstruct.empty(pCell);
      var ObjList = [{ 'value': 'OR', 'label': this.nls.inputs.optionOR }, { 'value': 'AND', 'label': this.nls.inputs.optionAND }];
    ...
    removeTableRow: function removeTableRow(pCell, pRow, pCount) {
      if (pCount > 1) {
    ...
    reconstructRows: function reconstructRows(pValue) {
      if (pValue !== "") {
        array.forEach(this.config.groups, lang.hitch(this, function (group) {
    ...
    checkDefaultValue: function checkDefaultValue(pGroup) {
      var defaultVal = "";
    ...
    checkDefaultOperator: function checkDefaultOperator(pGroup) {
      var defaultOp = "=";
      if (pGroup.operator !== "") {
    ...
    setFilterLayerDef: function setFilterLayerDef() {
      this.filterExt = null;
      this.graphicsHolder = [];
            ...
            array.forEach(group.layers, lang.hitch(this, function (grpLayer, i) {
              ...
              if (layer.id === grpLayer.layer) {
                ...
                array.forEach(sqlParams, lang.hitch(this, function (p) {
                  array.forEach(layer.layerObject.fields, lang.hitch(this, function (field) {
                    if (field.name === grpLayer.field) {
                      if (field.type.indexOf("Integer") > -1 || field.type.indexOf("Double") > -1 || field.type.indexOf("Single") > -1) {
    ...
    updateGroupDesc: function updateGroupDesc(pParam) {
      array.forEach(this.config.groups, lang.hitch(this, function (group) {
    ...
    toggleSaveFilter: function toggleSaveFilter() {
      var containerNode;
    ...
    saveJsonToFile: function saveJsonToFile() {
    ...
    readJsonToConfig: function readJsonToConfig() {
    ...
    _publishData: function _publishData(pValue) {
      var fieldList = [];
      var valueList = [];
    ...
    _queryExtentToZoom: function _queryExtentToZoom(results) {
