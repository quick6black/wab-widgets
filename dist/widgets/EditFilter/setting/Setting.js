...
define(['dojo/_base/declare', 'jimu/BaseWidgetSetting', 'dijit/_WidgetsInTemplateMixin', 'jimu/dijit/SimpleTable', 'dojo/dom', 'dojo/dom-construct', 'dojo/dom-class', 'dojo/on', 'dojo/query', 'dojo/dom-attr', 'dojo/_base/lang', 'dojo/_base/array', 'dijit/form/Select', 'dijit/form/TextBox', 'dijit/form/ValidationTextBox', 'dijit/form/CheckBox', 'jimu/utils', 'jimu/LayerInfos/LayerInfos', 'jimu/dijit/Message', 'jimu/dijit/Popup', 'dojox/html/entities', '../LayersHandler', './presetValuePicker'], function (declare, BaseWidgetSetting, _WidgetsInTemplateMixin, SimpleTable, dom, domConstruct, domClass, on, query, domAttr, lang, array, Select, TextBox, ValidationTextBox, CheckBox, utils, LayerInfos, Message, Popup, entities, LayersHandler, presetValuePicker) {
  return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
    ...
    createGroupBlock: function createGroupBlock(pParam) {
      this.groupCounter++;
    ...
    createAppendConjunc: function createAppendConjunc(params) {
    ...
    addLayerRow: function addLayerRow(pBlock, pParam) {
    ...
    createLayerSelection: function createLayerSelection(tr, pParam, pCounter) {
      var ctlLayerList = [];
    ...
    createFieldSelection: function createFieldSelection(pLayer, pTR, pParam, pCounter) {
      var ctlfieldList = [];
    ...
    */
    ...
    dataTypeSync: function dataTypeSync(pParam) {
      var dtSelection = pParam.select;
      dtSelection.set('value', pParam.field);
    },

    resetRadio: function resetRadio(pParam) {
      var row = pParam.row;
      var valueRadio = row.cells[2].childNodes[0];
    ...
    validateNoGroups: function validateNoGroups() {
      var validForm = false;
    ...
    validateNoGroupsName: function validateNoGroupsName() {
      var validForm = true;
    ...
    validateDuplicateGroupsName: function validateDuplicateGroupsName() {
      var validForm = true;
      ...
      if (names.length > 1 && names.sort().filter(function (v, i, o) {
        return v !== o[i - 1];
      }).length !== names.length) {
        validForm = false;
      }
    ...
    validateTableRows: function validateTableRows() {
      var validForm = true;
    ...
    presetPickerPopup: function presetPickerPopup(pInput) {
    ...
    removeGroup: function removeGroup(pBlock) {
      var numPart = pBlock.substring(pBlock.indexOf('_') + 1);
