...
define(['dojo/_base/declare', 'dojo/_base/lang', 'dojo/_base/array', 'esri/geometry/Extent', 'jimu/utils', 'esri/dijit/AttributeInspector'], function (declare, lang, array, Extent, jimuUtils, AttributeInspector) {
  ...
  mo.pointToExtent = function (map, point, toleranceInPixel) {
    ...
    return new Extent(point.x - toleranceInMapCoords, point.y - toleranceInMapCoords, point.x + toleranceInMapCoords, point.y + toleranceInMapCoords, map.spatialReference);
  };
  ...
  mo.filterOnlyUpdatedAttributes = function (attributes, origAttributes, featureLayer) {
    ...
    for (var prop in attributes) {
      if (attributes.hasOwnProperty(prop)) {
        if (prop === featureLayer.objectIdField || prop === featureLayer.globalIdField) {
      ...
      array.forEach(popupInfo.fieldInfos, function (fieldInfo) {
        array.some(layerInfo.layerObject.fields, function (field) {
          if (field.name === fieldInfo.fieldName) {
            ...
            if (mergedFieldInfo.format && mergedFieldInfo.format.dateFormat && mergedFieldInfo.format.dateFormat.toLowerCase() && mergedFieldInfo.format.dateFormat.toLowerCase().indexOf('time') >= 0) {
  ...
  mo.getFieldInfosLayer = function (layerInfo) {
    ...
    if (layerInfo && layerInfo.layerObject) {
      array.forEach(layerInfo.layerObject.fields, function (field) {
        ...
        //  }
        //}
        if (fieldInfo.format && fieldInfo.format.dateFormat && fieldInfo.format.dateFormat.toLowerCase() && fieldInfo.format.dateFormat.toLowerCase().indexOf('time') >= 0) {
  ...
  mo.getConfigInfos = function (jimuLayerInfos, configurationLayerInfos, editableOnly, configuredOnly) {
    ...
    array.forEach(jimuLayerInfos.getLayerInfoArrayOfWebmap(), function (layerInfo) {
      ...
      if (addLayer === true) {
        ...
        if (mergedConfigInfo.featureLayer.layerAllowsDelete === true && mergedConfigInfo.featureLayer.layerAllowsCreate === false && mergedConfigInfo.featureLayer.layerAllowsUpdate === false) {
  ...
  mo.createDefaultConfigInfo = function (layerInfo) {
    ...
    try {
      ...
      if (layerInfo.layerObject.hasOwnProperty('capabilities')) {
        if (String(layerInfo.layerObject.capabilities).indexOf('Update') === -1 && String(layerInfo.layerObject.capabilities).indexOf('Delete') === -1 && String(layerInfo.layerObject.capabilities).indexOf('Create') === -1 && String(layerInfo.layerObject.capabilities).indexOf('Editing') !== -1) {
          allowsUpdate = true;
          allowsDelete = true;
          allowsCreate = true;
        } else {
