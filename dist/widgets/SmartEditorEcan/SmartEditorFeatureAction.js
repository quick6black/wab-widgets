...
define(['dojo/_base/declare', 'dojo/_base/array', 'dojo/_base/lang', 'dojo/Deferred', 'jimu/BaseFeatureAction', 'jimu/Role', 'jimu/WidgetManager', 'dojo/query', 'jimu/PopupManager', 'dojo/aspect'], function (declare, array, lang, Deferred, BaseFeatureAction, Role, WidgetManager, query, PopupManager, aspect) {
  var clazz = declare(BaseFeatureAction, {
     ...
     */
    _isRelatedRecordDisplayed: function _isRelatedRecordDisplayed() {
     ...
     */
    _isLayerConfigured: function _isLayerConfigured(layerId) {
      var showOption;
      showOption = false;
      ...
      array.forEach(this._viewedFeatureIdArr, lang.hitch(this, function (viewedFeatureId, index) {
        if (!this._layerFoundDetail[viewedFeatureId]) {
          ...
          if (index > 0) {
            ...
            if (this._layerFoundDetail[this._viewedFeatureIdArr[index - 1]] && this._layerFoundDetail[this._viewedFeatureIdArr[index - 1]].relationshipInfos) {
              showOption = this._showWidgetFeatureAction(this._layerFoundDetail[this._viewedFeatureIdArr[index - 1]].relationshipInfos, viewedFeatureId);
            } else {
              showOption = false;
            }
     ...
     */
    _detectInfoWindowBackBtnClick: function _detectInfoWindowBackBtnClick() {
      ...
      if (popupManagerInstance) {
        ...
        if (relatedRecordsPopupProjectorInstance) {
          this._isInfoWindowBackBtnClickHandle = aspect.before(relatedRecordsPopupProjectorInstance, "_onPreviouBtnClick", lang.hitch(this, function () {
    ...
    onExecute: function onExecute(featureSet, layerParam) {
      ...
      WidgetManager.getInstance().triggerWidgetOpen(this.widgetId).then(lang.hitch(this, function (smartEditor) {
        smartEditor.beginEditingByFeatures(featureSet.features, layer, this._viewedFeatureIdArr, this._viewedFeatureArr);
      }));
      return def.promise;
    },

    _checkEditPrivilege: function _checkEditPrivilege(user) {
    ...
    _showWidgetFeatureAction: function _showWidgetFeatureAction(layerInfos, viewedFeatureId) {
      ...
      if (showOption) {
        ...
        if (showOption.hasOwnProperty("_editFlag")) {
          ...
          if (!showOption._editFlag && showOption.relationshipInfos && showOption.relationshipInfos.length > 0) {
            showRelatedLayer = this._hasAnyEditableLayerInRelation(showOption.relationshipInfos);
          }
          if (!showRelatedLayer) {
            showOption = false;
          }
